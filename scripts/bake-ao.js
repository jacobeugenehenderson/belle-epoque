/**
 * Fast Ambient Occlusion Baker for Belle Epoque
 *
...
const AO_SAMPLES = 24;        // Rays per vertex (reduced for speed)
const AO_DISTANCE = 20;       // Max ray distance
const AO_BIAS = 0.15;         // Offset to avoid self-intersection
const NEIGHBOR_RADIUS = 50;   // Only consider buildings within this radius
...
// Rotate sample direction to align with normal
...
// Find neighbors for a building (within NEIGHBOR_RADIUS)
function findNeighbors(buildingId, x, z) {
  const neighbors = [groundMesh]; // Always include ground
...
for (const building of buildingsData.buildings) {
  ...
  // Find neighboring meshes for this building
